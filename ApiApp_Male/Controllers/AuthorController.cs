...
namespace ApiApp_Male.Controllers
{
    ...
    public class AuthorController : ControllerBase
    {
        ...
        public AuthorController(IMapper Mapper, LibraryDBContext dbContext)
        {
            mapper = Mapper;
        ...
        {
            Author CurAuthor = dbContext.Authors.Where(x => x.AuthorId == Id && x.IsDeleted == false).FirstOrDefault();
            if (CurAuthor == null)
        ...
        public IActionResult AddAuthor([FromBody]AuthorAddRequestDTO NewAuthor)
        {
            ...
            if (dbContext.Authors.Any(x => x.AuthorName == NewAuthor.AuthorName))
            {
                ModelState.AddModelError("AuthorName", "Duplicate Name");
          ...
          /*  if (String.IsNullOrEmpty(NewAuthor.AuthorName))
           ...
           // CurAuthor.AuthorId = dbContext.Authors.Max(x => x.AuthorId) + 1;
            dbContext.Authors.Add(CurAuthor);
        ...
        public IActionResult UpdateAuthor(int AuthorId,AuthorUpdateRequestDTO newAuthor)
        {
            ...
            var CurAuthor = dbContext.Authors.Where(x => x.AuthorId == AuthorId).SingleOrDefault();
            if (CurAuthor == null)
            {
                return NotFound(new
                {
